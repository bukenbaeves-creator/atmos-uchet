...
async function main() {
  ...
  // Переименование системного компонента наркоза (седация выделена в отдельный компонент).
  await prisma.calcComponent.updateMany({ where: { code: 'anesthesia', name: 'Наркоз / седация' }, data: { name: 'Наркоз (общий)' } });
